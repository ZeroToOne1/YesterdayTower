...
public class BulletBehaviorAI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (gameObject.transform.position.Equals(targetPosition))
        {
            if (target != null)
            {
                ...
                {
                    Debug.Log("先减速");
                    if(target.GetComponent<AIPath>().maxSpeed * 0.8f > 0.8f)
                    {
                        target.GetComponent<AIPath>().maxSpeed = target.GetComponent<AIPath>().maxSpeed * 0.9f;

                    }
                }
                else
                {
                    target.GetComponent<AIPath>().maxSpeed = 0f;
                }
